...
def merge_persons(
    ...
    Steps:
    1. Combine bboxes, detection masks, and confidences via merge_identity_tracks()
    2. Update primary track in all_tracks with merged data
    3. Archive secondary person dir (rename to person_N.archived/)
    4. Remove secondary from all_tracks list
    5. Re-run isolation + GVHMR for merged primary person
    6. Recompute confidence + identity + bridging
    7. Return merge_record for undo
    """
    from identity_tracking import merge_identity_tracks, IdentityTrack, auto_generate_keyframes
    from identity_confidence import compute_all_confidences, export_confidence_csv, confidence_to_array
    from smplx_to_bvh import extract_gvhmr_params

    primary_dir = Path(person_dirs[primary_index])
    secondary_dir = Path(person_dirs[secondary_index])

    primary_track = all_tracks[primary_index]
    secondary_track = all_tracks[secondary_index]

    # Extract bboxes
    p_bboxes = primary_track["bbx_xyxy"]
    s_bboxes = secondary_track["bbx_xyxy"]
    if isinstance(p_bboxes, torch.Tensor):
        p_bboxes = p_bboxes.numpy()
    if isinstance(s_bboxes, torch.Tensor):
        s_bboxes = s_bboxes.numpy()

    # Detection masks
    p_mask = primary_track.get("detection_mask", np.ones(len(p_bboxes), dtype=bool))
    s_mask = secondary_track.get("detection_mask", np.ones(len(s_bboxes), dtype=bool))
    if isinstance(p_mask, torch.Tensor):
        p_mask = p_mask.numpy()
    if isinstance(s_mask, torch.Tensor):
        s_mask = s_mask.numpy()

    num_frames = max(len(p_bboxes), len(s_bboxes))

    # Load identity tracks from disk
    p_id_path = primary_dir / "identity_track.json"
    s_id_path = secondary_dir / "identity_track.json"
    p_id_track = IdentityTrack.load_json(p_id_path) if p_id_path.exists() else IdentityTrack(person_id=primary_track.get("track_id", primary_index))
    s_id_track = IdentityTrack.load_json(s_id_path) if s_id_path.exists() else IdentityTrack(person_id=secondary_track.get("track_id", secondary_index))

    # Compute confidences for both
    p_confs = compute_all_confidences(primary_index, all_tracks, num_frames)
    s_confs = compute_all_confidences(secondary_index, all_tracks, num_frames)

    # Pad bboxes/masks to num_frames
    if len(p_bboxes) < num_frames:
        p_bboxes = np.pad(p_bboxes, ((0, num_frames - len(p_bboxes)), (0, 0)))
        p_mask = np.pad(p_mask, (0, num_frames - len(p_mask)))
    if len(s_bboxes) < num_frames:
        s_bboxes = np.pad(s_bboxes, ((0, num_frames - len(s_bboxes)), (0, 0)))
        s_mask = np.pad(s_mask, (0, num_frames - len(s_mask)))

    if progress_callback:
        progress_callback(0.1, "Merging tracks...")

    # 1. Merge via identity_tracking
    merged_track, merged_bboxes, merged_mask, merged_confs = merge_identity_tracks(
        primary=p_id_track,
        secondary=s_id_track,
        primary_bboxes=p_bboxes,
        secondary_bboxes=s_bboxes,
        primary_mask=p_mask,
        secondary_mask=s_mask,
        primary_confidences=p_confs,
        secondary_confidences=s_confs,
        num_frames=num_frames,
    )

    # Save backup of primary's original bboxes for undo
    merge_record = {
        "operation": "merge",
        "primary_index": primary_index,
        "secondary_index": secondary_index,
        "primary_original_bboxes": p_bboxes.copy(),
        "primary_original_mask": p_mask.copy(),
        "secondary_track_id": secondary_track.get("track_id", secondary_index),
        "archived_dir": str(secondary_dir) + ".archived",
    }

    # 2. Update primary track data
    primary_track["bbx_xyxy"] = torch.from_numpy(merged_bboxes)
    if "detection_mask" in primary_track:
        primary_track["detection_mask"] = torch.from_numpy(merged_mask)

    # 3. Archive secondary dir
    archived_path = Path(str(secondary_dir) + ".archived")
    if secondary_dir.exists():
        if archived_path.exists():
            shutil.rmtree(str(archived_path))
        secondary_dir.rename(archived_path)

    if progress_callback:
        progress_callback(0.3, "Re-running isolation + GVHMR for merged person...")

    # 5. Re-run isolation + GVHMR for merged primary
    reprocess_result = reprocess_person(
        video_path=video_path,
        person_index=primary_index,
        person_dir=str(primary_dir),
        updated_bboxes=merged_bboxes,
        all_tracks=all_tracks,
        slam_path=slam_path,
        masks_dir=masks_dir,
        static_cam=static_cam,
    ...
    merge_record["reprocess_result"] = reprocess_result
    merge_record["merged_identity_track"] = reprocess_result.get("identity_track", merged_track)
    merge_record["merged_confidences"] = reprocess_result.get("confidences", merged_confs)
...
def split_person(
    ...
    """Split person at split_frame into two tracks.

    1. Split bboxes: person_A = [0, split_frame), person_B = [split_frame, end)
    2. Split detection_mask and detection_conf
    3. Create new person dir for person_B
    4. Add new track to all_tracks
    5. Re-run isolation + GVHMR for both
    6. Return split_record for undo
    """
    from identity_tracking import split_identity_track, IdentityTrack, auto_generate_keyframes
    from identity_confidence import compute_all_confidences, export_confidence_csv

    original_track = all_tracks[person_index]
    original_dir = Path(person_dirs[person_index])

    bboxes = original_track["bbx_xyxy"]
    if isinstance(bboxes, torch.Tensor):
        bboxes = bboxes.numpy()
    num_frames = len(bboxes)

    # Determine new person ID
    existing_ids = [t.get("track_id", i) for i, t in enumerate(all_tracks)]
    new_pid = max(existing_ids) + 1

    # Split bboxes and masks
    bboxes_a = bboxes.copy()
    bboxes_b = bboxes.copy()
    # Zero out frames outside each half
    bboxes_a[split_frame:] = 0
    bboxes_b[:split_frame] = 0

    mask = original_track.get("detection_mask")
    if mask is not None:
        if isinstance(mask, torch.Tensor):
            mask = mask.numpy()
        mask_a = mask.copy()
        mask_b = mask.copy()
        mask_a[split_frame:] = False
        mask_b[:split_frame] = False
    else:
        mask_a = np.ones(num_frames, dtype=bool)
        mask_b = np.ones(num_frames, dtype=bool)
        mask_a[split_frame:] = False
        mask_b[:split_frame] = False

    conf = original_track.get("detection_conf")
    if conf is not None:
        if isinstance(conf, torch.Tensor):
            conf = conf.numpy()
        conf_a = conf.copy()
        conf_b = conf.copy()
        conf_a[split_frame:] = 0
        conf_b[:split_frame] = 0
    else:
        conf_a = mask_a.astype(np.float32)
        conf_b = mask_b.astype(np.float32)

    if progress_callback:
        progress_callback(0.1, "Splitting track...")

    # Update original track (before-half)
    original_track["bbx_xyxy"] = torch.from_numpy(bboxes_a)
    if "detection_mask" in original_track:
        original_track["detection_mask"] = torch.from_numpy(mask_a)
    if "detection_conf" in original_track:
        original_track["detection_conf"] = torch.from_numpy(conf_a)

    # Create new track (after-half)
    new_track = {
        "track_id": new_pid,
        "bbx_xyxy": torch.from_numpy(bboxes_b),
        "detection_mask": torch.from_numpy(mask_b),
        "detection_conf": torch.from_numpy(conf_b),
    }

    # Create new person dir
    new_dir = Path(output_dir) / f"person_{len(person_dirs)}"
    ...
    split_record = {
        ...
        "new_person_index": len(all_tracks),
        "new_person_dir": str(new_dir),
        "original_bboxes": bboxes.copy(),
        "original_mask": mask.copy() if mask is not None else None,
        "original_conf": conf.copy() if conf is not None else None,
    }

    if progress_callback:
        progress_callback(0.2, "Reprocessing split persons...")

    # Reprocess person A (before-half)
    result_a = reprocess_person(
        video_path=video_path,
        person_index=person_index,
        person_dir=str(original_dir),
        updated_bboxes=bboxes_a,
        all_tracks=all_tracks,
        slam_path=slam_path,
        masks_dir=masks_dir,
        static_cam=static_cam,
        use_dpvo=use_dpvo,
        progress_callback=lambda f, m: progress_callback(0.2 + f * 0.35, m) if progress_callback else None,
    )

    # Add new track to all_tracks before reprocessing B
    all_tracks.append(new_track)

    # Reprocess person B (after-half)
    result_b = reprocess_person(
        video_path=video_path,
        person_index=len(all_tracks) - 1,
        person_dir=str(new_dir),
        updated_bboxes=bboxes_b,
        all_tracks=all_tracks,
        slam_path=slam_path,
        masks_dir=masks_dir,
        static_cam=static_cam,
        use_dpvo=use_dpvo,
        progress_callback=lambda f, m: progress_callback(0.55 + f * 0.35, m) if progress_callback else None,
    )

    split_record["result_a"] = result_a
    split_record["result_b"] = result_b
